...
type Props = {
  ...
  showExpandedThirdPartyOptions?: boolean
}

type PlatformDocsProvider = 'bedrock' | 'foundry' | 'vertex'
type ProviderInputProviderId = 'kimi-for-coding' | 'others'

type OAuthStatus =
  | { state: 'idle' }
  | { state: 'platform_setup' }
  | { state: 'platform_docs'; provider: PlatformDocsProvider }
  | { state: 'provider_base_url' }
  | {
      state: 'provider_api_key'
      providerId: ProviderInputProviderId
      providerName: string
      apiKeyUrl: string
...
const PLATFORM_DOCS: Record<
  PlatformDocsProvider,
  { title: string; description: string; url: string }
> = {
  bedrock: {
    title: 'Amazon Bedrock',
    description:
      'Configure the required AWS credentials and Bedrock environment variables, then continue with Claude Code.',
    url: 'https://code.claude.com/docs/en/amazon-bedrock',
  },
  foundry: {
    title: 'Microsoft Foundry',
    description:
      'Configure the required Microsoft Foundry resource settings and authentication before continuing.',
    url: 'https://code.claude.com/docs/en/microsoft-foundry',
  },
  vertex: {
    title: 'Google Vertex AI',
    description:
      'Configure your GCP project, region, and Vertex AI credentials before continuing.',
    url: 'https://code.claude.com/docs/en/google-vertex-ai',
  },
}

const KIMI_PROVIDER = {
  providerId: 'kimi-for-coding' as const,
...
function CopilotOAuthContent({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        1. Open: <Text color="cyan">{verificationUri}</Text>
      </Text>
      <Text>
        2. Enter code: <Text bold color="warning">{userCode}</Text>
      </Text>
      <Text dimColor>{status ?? 'Waiting for authorization... (Press Esc to go back)'}</Text>
    </Box>
  )
}
...
export function ConsoleOAuthFlow({
  ...
  showExpandedThirdPartyOptions = false,
  ...
  const pasteInputColumns = columns - PASTE_HERE_MSG.length - 1
  const baseUrlInputColumns = columns - BASE_URL_PROMPT.length - 1
  const apiKeyInputColumns = columns - API_KEY_PROMPT.length - 1
  ...
  const goBackToPlatformSetup = useCallback(() => {
    resetProviderInputs()
  ...
  useEffect(() => {
    if (oauthStatus.state === 'about_to_retry') {
      const timer = setTimeout(() => setOAuthStatus(oauthStatus.nextState), 1000)
  ...
  useKeybinding(
    'confirm:yes',
    () => {
      if (showExpandedThirdPartyOptions) {
        setOAuthStatus({ state: 'platform_setup' })
      } else {
    ...
    },
    {
      context: 'Confirmation',
      isActive:
        oauthStatus.state === 'platform_docs' ||
        (!showExpandedThirdPartyOptions && oauthStatus.state === 'platform_setup'),
    },
  )
  ...
  useKeybinding(
    'confirm:no',
    () => {
      if (!showExpandedThirdPartyOptions) return
      if (
        oauthStatus.state === 'provider_base_url' ||
        oauthStatus.state === 'provider_api_key' ||
        oauthStatus.state === 'copilot_oauth' ||
        oauthStatus.state === 'copilot_polling' ||
        oauthStatus.state === 'platform_docs'
      ) {
        goBackToPlatformSetup()
      }
    },
    {
      context: 'Confirmation',
      isActive:
        showExpandedThirdPartyOptions &&
        (oauthStatus.state === 'provider_base_url' ||
          oauthStatus.state === 'provider_api_key' ||
          oauthStatus.state === 'copilot_oauth' ||
          oauthStatus.state === 'copilot_polling' ||
  ...
  const persistConnectedProvider = useCallback(
    async (
      provider:
        | {
            providerId: ProviderInputProviderId
      ...
      try {
        if (provider.providerId === 'github-copilot') {
          ...
          fetchCopilotModelsFromModelsDev()
            .then(models => {
              saveGlobalConfig(current => ({
                ...current,
        ...
        if (provider.model) {
          envPatch.ANTHROPIC_MODEL = provider.model
          envPatch.ANTHROPIC_SMALL_FAST_MODEL = provider.model
        } else {
          envPatch.ANTHROPIC_MODEL = null
          envPatch.ANTHROPIC_SMALL_FAST_MODEL = null
        }

        saveGlobalConfig(current => ({
          ...current,
          hasCompletedOnboarding: true,
          env: mergeConfigEnv(current.env, envPatch),
          connectedProviders: {
            ...(current.connectedProviders || {}),
            [provider.providerId]: {
              apiKey: provider.apiKey,
              enterpriseUrl: provider.baseUrl,
              connectedAt: new Date().toISOString(),
            },
          },
          ...
          toRetry:
            provider.providerId === 'github-copilot'
              ...
              : {
                  ...
                  apiKeyUrl:
                    provider.providerId === 'kimi-for-coding'
                      ? KIMI_PROVIDER.apiKeyUrl
                      : 'Your provider console',
                  keyPlaceholder: provider.providerId === 'kimi-for-coding' ? 'sk-...' : 'token',
  ...
  const handleProviderApiKeySubmit = useCallback(
    async (value: string) => {
      if (oauthStatus.state !== 'provider_api_key') return
      ...
      await persistConnectedProvider({
        providerId: oauthStatus.providerId,
        providerName: oauthStatus.providerName,
        baseUrl: oauthStatus.baseUrl ?? '',
        apiKey,
        ...(oauthStatus.providerId === 'kimi-for-coding'
          ? { model: KIMI_PROVIDER.model }
          : {}),
      })
    },
    [oauthStatus, persistConnectedProvider],
  )

  const handleOthersBaseUrlSubmit = useCallback((value: string) => {
    try {
      ...
      setOAuthStatus({
        ...
        toRetry: { state: 'provider_base_url' },
      })
    }
  }, [])
  ...
  const startCopilotOAuth = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        setOAuthStatus({
          state: 'error',
          message: 'Failed to initiate GitHub device authorization',
      ...
      setOAuthStatus({
        ...
        message:
          error instanceof Error
            ? `Failed to connect GitHub Copilot: ${error.message}`
  ...
  useEffect(() => {
    if (oauthStatus.state !== 'copilot_oauth') return
  ...
  useEffect(() => {
    if (oauthStatus.state !== 'copilot_polling') return
      ...
      while (!stopped) {
        ...
        try {
          ...
          setOAuthStatus({
            ...
            message:
              error instanceof Error
                ? `GitHub Copilot network error: ${error.message}`
                : 'GitHub Copilot network error',
            toRetry: { state: 'platform_setup' },
          })
          return
        }
  ...
  switch (oauthStatus.state) {
        ...
        },
        {
          label: (
            <Text>
              3rd-party platform ·{' '}
              <Text dimColor>
                Amazon Bedrock, Microsoft Foundry, Vertex AI, GitHub Copilot, Kimi,
                or Others
              </Text>
              {'\n'}
            </Text>
    ...
    case 'platform_setup': {
      ...
      const thirdPartyOptions = [
        {
          label: (
            <Text>
              Amazon Bedrock · <Text dimColor>AWS-managed Anthropic access</Text>
              {'\n'}
            </Text>
          ),
          value: 'bedrock',
        },
        {
          label: (
            <Text>
              Microsoft Foundry ·{' '}
              <Text dimColor>Use Microsoft-hosted Anthropic endpoints</Text>
              {'\n'}
            </Text>
          ),
          value: 'foundry',
        },
        {
          label: (
            <Text>
              Google Vertex AI · <Text dimColor>Use Google-hosted Anthropic endpoints</Text>
              {'\n'}
            </Text>
          ),
          value: 'vertex',
        },
        {
          label: (
            <Text>
              GitHub Copilot · <Text dimColor>Connect with GitHub device OAuth</Text>
              {'\n'}
            </Text>
          ),
        ...
        },
        {
          label: (
            <Text>
              Kimi For Coding ·{' '}
              <Text dimColor>Use Moonshot&apos;s Anthropic-compatible endpoint</Text>
              {'\n'}
            </Text>
          ),
        ...
        },
        {
          label: (
            <Text>
              Others · <Text dimColor>Enter your own base URL and key</Text>
              {'\n'}
            </Text>
          ),
          value: 'others',
      ...
      content = (
        <Box flexDirection="column" gap={1} marginTop={1}>
          <Text bold>Using 3rd-party platforms</Text>
          <Text>Select a provider to configure during onboarding:</Text>
          <Box>
            <Select
              options={thirdPartyOptions}
              onChange={value => {
                if (value === 'bedrock' || value === 'foundry' || value === 'vertex') {
                  setOAuthStatus({ state: 'platform_docs', provider: value })
                  return
                }
                ...
                if (value === 'kimi-for-coding') {
                  setProviderBaseUrl(KIMI_PROVIDER.baseUrl)
                  setProviderApiKey('')
                  setProviderApiKeyCursorOffset(0)
                  setOAuthStatus({
                    state: 'provider_api_key',
                    providerId: KIMI_PROVIDER.providerId,
                    providerName: KIMI_PROVIDER.providerName,
                    apiKeyUrl: KIMI_PROVIDER.apiKeyUrl,
                    keyPlaceholder: KIMI_PROVIDER.keyPlaceholder,
                ...
                resetProviderInputs()
                setOAuthStatus({ state: 'provider_base_url' })
              }}
    ...
    case 'platform_docs': {
      const provider = PLATFORM_DOCS[oauthStatus.provider]
      content = (
        <Box flexDirection="column" gap={1} marginTop={1}>
          <Text bold>{provider.title}</Text>
          <Text>{provider.description}</Text>
          <Text>
            Documentation: <Link url={provider.url}>{provider.url}</Link>
          </Text>
          <Box marginTop={1}>
            <Text dimColor>
    ...
    case 'provider_base_url': {
      content = (
        <Box flexDirection="column" gap={1} marginTop={1}>
          <Text bold>Configure Others</Text>
          <Text>Enter the Anthropic-compatible base URL for your provider.</Text>
          <Text dimColor>Example: https://your-provider.example.com/anthropic</Text>
          <Box>
            <Text>{BASE_URL_PROMPT}</Text>
            <TextInput
              value={providerBaseUrl}
              onChange={setProviderBaseUrl}
              onSubmit={handleOthersBaseUrlSubmit}
              cursorOffset={providerBaseUrlCursorOffset}
              onChangeCursorOffset={setProviderBaseUrlCursorOffset}
              columns={baseUrlInputColumns}
            />
          </Box>
    ...
    case 'provider_api_key': {
      content = (
        <Box flexDirection="column" gap={1} marginTop={1}>
          <Text bold>Configure {oauthStatus.providerName}</Text>
          {oauthStatus.baseUrl ? (
            <Text dimColor>
              Base URL: <Text>{oauthStatus.baseUrl}</Text>
            </Text>
          ) : null}
          <Text dimColor>
            Get your API key from: <Text color="cyan">{oauthStatus.apiKeyUrl}</Text>
          </Text>
          <Box>
            <Text>{API_KEY_PROMPT}</Text>
            <TextInput
              value={providerApiKey}
              onChange={setProviderApiKey}
              onSubmit={value => void handleProviderApiKeySubmit(value)}
              cursorOffset={providerApiKeyCursorOffset}
              onChangeCursorOffset={setProviderApiKeyCursorOffset}
          ...
          {showPastePrompt ? (
            <Box>
              ...
              <TextInput
                ...
                onSubmit={value => void handleSubmitCode(value, oauthStatus.url)}
                cursorOffset={cursorOffset}
                onChangeCursorOffset={setCursorOffset}
                columns={pasteInputColumns}
                mask="*"
              />
            </Box>
          ) : null}
    ...
    case 'copilot_polling': {
      content = (
        <CopilotOAuthContent
          userCode={oauthStatus.userCode}
          verificationUri={oauthStatus.verificationUri}
          status="Waiting for authorization... (Press Esc to go back)"
        />
      )
      break
    }
