...
import {
  ...
  getOriginalClaudeConfigHomeDir,
  isBareMode,
  isEnvTruthy,
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const claudeUserSkillsDir = join(
      getOriginalClaudeConfigHomeDir(),
      'skills',
    )
    const cursorUserSkillsDir = join(homedir(), '.cursor', 'skills')
    const codexUserSkillsDir = join(homedir(), '.codex', 'skills')
    const userSkillsDir = join(getClaudeConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.claude', 'skills')
    const projectSkillsDirs = getProjectDirsUpToHome('skills', cwd)

    logForDebugging(
      `Loading skills from: managed=${managedSkillsDir}, user=${userSkillsDir}, claudeUser=${claudeUserSkillsDir}, cursorUser=${cursorUserSkillsDir}, codexUser=${codexUserSkillsDir}, project=[${projectSkillsDirs.join(', ')}]`,
    )
    ...
    const [
      ...
      isSettingSourceEnabled('userSettings') && !skillsLocked
        ...
        : Promise.resolve([]),
      isSettingSourceEnabled('userSettings') && !skillsLocked
        ? loadSkillsFromSkillsDir(claudeUserSkillsDir, 'userSettings')
        : Promise.resolve([]),
      isSettingSourceEnabled('userSettings') && !skillsLocked
        ? loadSkillsFromSkillsDir(cursorUserSkillsDir, 'userSettings')
        : Promise.resolve([]),
      isSettingSourceEnabled('userSettings') && !skillsLocked
        ? loadSkillsFromSkillsDir(codexUserSkillsDir, 'userSettings')
        : Promise.resolve([]),
    ...
    logForDebugging(
      `Loaded ${deduplicatedSkills.length} unique skills (${unconditionalSkills.length} unconditional, ${newConditionalSkills.length} conditional, managed: ${managedSkills.length}, user: ${userSkills.length}, claude user: ${claudeUserSkills.length}, cursor user: ${cursorUserSkills.length}, codex user: ${codexUserSkills.length}, project: ${projectSkillsNested.flat().length}, additional: ${additionalSkillsNested.flat().length}, legacy commands: ${legacyCommands.length})`,
    )
