 ...
 *
 * Reads the configuration file at ~/.openclaude/plugins/known_marketplaces.json
 ...
 *
 * Writes the configuration to ~/.openclaude/plugins/known_marketplaces.json,
 ...
 *
 * Cache structure:
 * ~/.openclaude/plugins/marketplaces/
 ...
 * The configuration is saved to ~/.openclaude/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: openclaude plugin disable <plugin>@${name}`,
    )
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Claude Code version. Run 'openclaude plugin marketplace remove ${name}' and ` +
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: openclaude plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
      ...
      try {
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: openclaude plugin marketplace remove "${name}"`,
        )
      }
