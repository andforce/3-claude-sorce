...
export type GlobalConfig = {
  ...
  // Timestamp when changelog was last fetched (content stored in ~/.openclaude/cache/changelog.md)
  changelogLastFetched?: number
  // @deprecated - Migrated to ~/.openclaude/cache/changelog.md. Keep for migration support.
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    // overwrites a good backup. Backups are stored in ~/.openclaude/backups/ to
 ...
 * Uses ~/.openclaude/backups/ to keep the home directory clean.
 */
 ...
 * Checks ~/.openclaude/backups/ first, then falls back to the legacy location
