 ...
 * Path format: ~/.openclaude/projects/{sanitized-cwd}/...
 */
 ...
 * resolution, paths like /tmp/openclaude-{uid}/... wouldn't match /private/tmp/openclaude-{uid}/...
 */
 ...
 * Path format: /tmp/openclaude-{uid}/{sanitized-cwd}/
 */
 ...
 * Path format: /tmp/openclaude-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/openclaude-0/proj/session/scratchpad/../../../etc/passwd
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // Accepts both the broad patterns ('/.claude/**', '~/.openclaude/**') and
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.openclaude/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.openclaude is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.openclaude/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .claude/ only (not ~/.openclaude/) since launch.json is per-project.
  if (
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/openclaude-{uid}/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.openclaude/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.openclaude/teams/) for swarm coordination
