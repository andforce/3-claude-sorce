 ...
 * =====================
 * This module manages an isolated tmux socket for OpenClaude's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, OpenClaude could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started OpenClaude from within tmux.
 *
 * HOW IT WORKS:
 * 1. OpenClaude creates its own tmux socket: `openclaude-<PID>` (e.g., `openclaude-12345`)
 ...
 *
 * This means ANY tmux command run through OpenClaude - whether via the Tmux tool
 * directly or via Bash - will operate on OpenClaude's isolated socket, NOT the
 * user's tmux session.
 *
...
const CLAUDE_SOCKET_PREFIX = 'openclaude'
 ...
 * Example: "/tmp/tmux-501/openclaude-12345,54321,0"
 *
